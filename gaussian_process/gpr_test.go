...
func TestGaussianProcessRegressor_Fit(t *testing.T) {
	...
	{
		x:=X.RawMatrix().Data
		for i:=range x{x[i]=.1+((9.9-.1)*float64(i)/(float64(len(x)-1)))}
	}
	//np.random.seed(1)
	...
	dy:=make([]float64,20)
	{
		for i:=range dy{dy[i]=0.5 + 1.0 * randomState.Float64()}
	}
	noise:=make([]float64,20)
	{
		for i:=range noise{noise[i]=randomState.NormFloat64()*dy[i]}
	}
