...
func (m *GaussianProcessRegressor) LogMarginalLikelihood(Theta mat.Matrix, evalGradient bool) (
	...
	sub1:=0.
	for i:=0;i<ry;i++ {sub1+=math.Log(L.At(i,i))}
	sub2:=float64(nx)/2*math.Log(2*math.Pi)
	for i:=0;i<cy;i++ {lldims.SetVec(i,-.5*lldims.AtVec(i)-sub1-sub2)}
	...
	if evalGradient {
		...
		llgdims:= mat.NewVecDense(nk,nil)
		_=llgdims
		// TODO ...
	}
	return
