...
func TestKMeans(t *testing.T) {
	...
	X.Apply(func(i, j int, v float64) float64 { return 1. + rand.Float64() }, X)
	...
	getData := func(value int) (scatterData plotter.XYs) {
		for i := 0; i < NSamples; i++ {
			if int(Y.At(i, 0)) == value {
				scatterData = append(scatterData, struct{ X, Y float64 }{X.At(i, 0), X.At(i, 1)})
			}
		}
		return
	}
	...
	if canPlot() {
		...
		for cl := 0; cl < kmeans.NClusters; cl++ {
			...
			p.Legend.Add(fmt.Sprintf("scatter %d", cl), s)
			{
				...
				sc, err := plotter.NewScatter(plotter.XYs{{X: c.AtVec(0), Y: c.AtVec(1)}})
				if err != nil {
					panic(err)
				}
